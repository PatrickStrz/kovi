...
class ChallengeListTing extends Component {
  state = { commentsId:''}
  render(){
    ...
    return(
        ...
        <div>
          ...
          return(<div onClick={()=>{this.setState({commentsId:challenge.id})}}>{challenge.id}</div>)
        })}
        <ChallengeCommentsContainer challengeId={this.state.commentsId} />
      </div>
    )
  }
}
