...
class Burst extends Component{
  ...
  componentDidMount(){
    ...
    const element = document.getElementById(this.props.parentElementId)
...
class MoUpvote extends Component{
  ...
  render(){
    const {userDidUpvote, uniqueId} = this.props
    ...
    return(
      <div>
        <div style={{cursor:'pointer'}} id={uniqueId} onClick={this.props.handleClick}
          className={this.state.clicked ? "icon-container" : "icon-container upvote"}>
          <span style={styles.icon} className={this.props.animate ? "fa fa-chevron-up" : "fa fa-chevron-up"}></span></div>
        {this.props.animate && <Burst parentElementId={uniqueId}/>}
        {/* {this.props.animate && <Burst/>} */}
      </div>
    )
  }
}
