...
class Burst extends Component{
  ...
  render(){
    ...
    <div className="upvote"><span className="fa fa-arrow-up upvote"></span></div>
    )
  }
}
...
class MoUpvote extends Component{
  ...
  render(){
    return(
      <div>
        {this.state.clicked ? <Burst/> : <div onClick={()=>this.setState({clicked:!this.state.clicked})} className="upvote"><span className="fa fa-arrow-up upvote"></span></div>}
      </div>
    )
  }
}
